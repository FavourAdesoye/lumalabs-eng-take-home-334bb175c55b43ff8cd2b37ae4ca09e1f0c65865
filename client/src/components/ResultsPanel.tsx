...
export function ResultsPanel({
  ...
  const statusLabel = semanticEnabled
    ? 'Hybrid reranking on'
    : semanticFallback
      ? 'Lexical fallback'
      : 'Ready to search'
    ...
    <section className="results-panel">
      <div className="results-panel__header">
        ...
        <div className="results-panel__status">
          <div className={semanticEnabled ? 'status-pill is-active' : semanticFallback ? 'status-pill is-fallback' : 'status-pill'}>
            {statusLabel}
          </div>
          <span className="query-timing">{loading ? 'Searching...' : `${queryTimeMs.toFixed(1)} ms`}</span>
        </div>
      </div>
          ...
          <article key={result.id} className="result-card">
            ...
            <div className="result-card__footer">
              ...
              <div className="score-breakdown">
                ...
                <span>
                  Semantic {result.semanticScore === null ? 'n/a' : result.semanticScore.toFixed(2)}
                </span>
              </div>
            </div>
          </article>
